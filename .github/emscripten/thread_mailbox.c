...
void _emscripten_check_mailbox() {
  ...
  // Actually run the synchronization the comment above promises: replay the
  // dlopen/dlsym events that landed while this thread was parked so a proxied
  // task can't make an indirect call into a side module whose table slots are
  // still empty on this thread (as invokeEntryPoint does before its entry-point
  // call). Not valid on the main runtime thread, which catches up via the async
  // path instead, so guard it the same way emscripten_futex_wait.c does.
